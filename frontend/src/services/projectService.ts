  ...
  async getProjects(clientEmail?: string): Promise<Project[]> {
    if (isMockMode()) {
      ...
      if (clientEmail) {
        ...
        return mockProjects.filter(p => p.clientEmail.toLowerCase() === normalized);
      }
      return [...mockProjects];
    }
    const query = clientEmail ? `?email=${encodeURIComponent(clientEmail)}` : '';
  ...
  async createOrder(data: {
    ...
    if (isMockMode()) {
      ...
      const newProject: Project = {
        ...
        status: 'Uploaded',
        productImageUrl: data.productImageUrl || 'https://images.unsplash.com/photo-1555396273-367ea4eb4db5?auto=format&fit=crop&w=800&q=80',
  ...
  async updateProjectStatus(id: string, status: ProjectStatus): Promise<Project> {
    if (isMockMode()) {
      ...
      if (status === 'Completed' && !updated.arViewerUrl) {
        ...
        updated.arModelUrl = updated.arModelUrl || `https://assets.immversestudios.com/models/${id}.glb`;
      }

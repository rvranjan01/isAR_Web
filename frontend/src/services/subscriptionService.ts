...
export const subscriptionService = {
  ...
  async getSubscriptionByEmail(clientEmail: string): Promise<Subscription | null> {
    if (isMockMode()) {
      await new Promise(res => setTimeout(res, 200));
      const normalized = clientEmail.toLowerCase();
      const sub = mockSubscriptions.find(s => s.clientEmail.toLowerCase() === normalized);
      if (sub) return sub;
    ...
    return fetchClient<Subscription>(`/api/subscriptions/client?email=${encodeURIComponent(clientEmail)}`);
  },
  ...
  async requestRenewal(clientEmail: string): Promise<Subscription> {
    if (isMockMode()) {
      ...
      const normalized = clientEmail.toLowerCase();
      const index = mockSubscriptions.findIndex(s => s.clientEmail.toLowerCase() === normalized);
      if (index === -1) throw new Error('Subscription not found');

      const current = mockSubscriptions[index];
      if (current.status === 'renewal_requested') {
        throw new Error('Renewal already requested. Awaiting admin confirmation.');
      }
  ...
  async confirmRenewal(id: string): Promise<Subscription> {
    ...
    return fetchClient<Subscription>(`/api/subscriptions/${id}/confirm-renewal`, {
      method: 'POST'
    });
  },

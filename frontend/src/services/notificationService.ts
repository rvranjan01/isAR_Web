...
export const notificationService = {
  async getNotifications(recipientEmail: string): Promise<Notification[]> {
    if (isMockMode()) {
      ...
      return mockNotifications.filter(n => n.recipientEmail.toLowerCase() === normalized || normalized.includes('admin'));
    }
    return fetchClient<Notification[]>(`/api/notifications?email=${encodeURIComponent(recipientEmail)}`);
  },
  ...
  async markAllAsRead(recipientEmail: string): Promise<void> {
    if (isMockMode()) {
      ...
      mockNotifications = mockNotifications.map(n => 
        n.recipientEmail.toLowerCase() === normalized ? { ...n, read: true } : n
      );
      return;
    }
